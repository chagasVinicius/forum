...
func (q *Queries) GetRoomMessages(ctx context.Context, arg GetRoomMessagesParams) ([]uuid.UUID, error) {
	rows, err := q.db.Query(ctx, getRoomMessages, arg.RoomID, arg.Message)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
